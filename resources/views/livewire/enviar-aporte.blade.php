<div class="max-w-2xl bg-white p-6 shadow-sm sm:rounded-lg">
    ...
    @if($resultado) <p class="mt-3 rounded p-3 text-sm {{ $resultado === 'aprobado' ? 'bg-emerald-50 text-emerald-800' : 'bg-amber-50 text-amber-800' }}">{{ $resultado === 'aprobado' ? "El dato fue aplicado y se acreditaron {$recompensaAcreditada} créditos a tu cuenta." : 'El dato ya existe y quedó pendiente de revisión.' }}</p> @endif
