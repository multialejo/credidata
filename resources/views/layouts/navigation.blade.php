        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a data-testid="home-link" href="{{ auth()->user()->staff ? route('admin.clientes') : route('dashboard') }}">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    @if(auth()->user()->staff)
                        ...
                        <x-nav-link :href="route('admin.registros')" :active="request()->routeIs('admin.registros')">
                            {{ __('Registros') }}
                        </x-nav-link>
                    ...
                    @else
                        ...
                        <x-nav-link :href="route('dashboard.consultas')" :active="request()->routeIs('dashboard.consultas')">
                            {{ __('Historial') }}
                        </x-nav-link>
                        <x-nav-link :href="route('dashboard.api-key')" :active="request()->routeIs('dashboard.api-key')">
                            {{ __('API Key') }}
                        </x-nav-link>
        ...
        <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
            <div class="pt-2 pb-3 space-y-1">
                @if(auth()->user()->staff)
                    ...
                    <x-responsive-nav-link :href="route('admin.config')" :active="request()->routeIs('admin.config')">
                        {{ __('Configuración') }}
                    </x-responsive-nav-link>
                    <x-responsive-nav-link :href="route('admin.registros')" :active="request()->routeIs('admin.registros')">
                        {{ __('Registros') }}
                    </x-responsive-nav-link>
                ...
                @else
                    ...
                    <x-responsive-nav-link :href="route('dashboard.consultas')" :active="request()->routeIs('dashboard.consultas')">
                        {{ __('Historial') }}
                    </x-responsive-nav-link>
                    <x-responsive-nav-link :href="route('dashboard.api-key')" :active="request()->routeIs('dashboard.api-key')">
                        {{ __('API Key') }}
                    </x-responsive-nav-link>
